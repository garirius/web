...
function ProjectElement(proyectus){
  ...
  this.creaDiv = function() {
    ...
    var barra = $("<div class='deco' style='background-color: "+colors[this.clase]+"'></div>");

    ildivo.append(ladesc);
    ildivo.append(barra);
    $(".latabla").append(ildivo);
    console.log(ildivo.children(".desc").children("H2").html());
...
function loadProjects(pagina){
  switch (pagina) {
    case "mps":
    case "minuteearth":
      $.getJSON("sueltas.json", function(data){
        var kappa = data[pagina];
        for(i in kappa){
          var ele = new ProjectElement(kappa[i]);
          ele.creaDiv();
        }
        responsiveProjects();
      });
      break;
    default:
    $.getJSON(pagina+".json",{async:true}, function(data){
      for(i in data){
